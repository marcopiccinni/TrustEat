...
from TrustEat.maps import distance_time  # Da lasciare in caso di attivazione
import datetime
...
class ListOrder(View):
    ...
    def get(self, request, cod_locale):
        ...
        if request.user.is_commerciante and request.user in \
            ...
            args = {'waiting_list': waiting_list, 'refused_list': refused_list, 'delivering_list': delivering_list,
                    'delivering_js': delivering_js, 'delivered_list': delivered_list, 'local': local,
...
def db_order_consistance():
    for order in OrdineInAttesa.objects.filter(accettato=None):
        if order.data.date() < datetime.datetime.now().date():
            OrdineInAttesa.objects.filter(pk=order.cod_ordine) \
